...
async function login() {
  ...
  try {
    ...
    errorText.value = error?.response?.data?.errors?.email?.[0] ?? 'Ошибка входа.';
  } finally {
...
<template>
  <div class="min-h-screen flex items-center justify-center relative overflow-hidden py-12">
    ...
    <div class="absolute -top-40 -right-40 w-96 h-96 bg-primary/10 rounded-full blur-[120px] pointer-events-none" />
    <div class="absolute -bottom-40 -left-40 w-96 h-96 bg-primary/5 rounded-full blur-[120px] pointer-events-none" />

    <div class="relative z-10 w-full max-w-lg mx-auto px-4">
      <div class="text-center mb-10">
        <RouterLink to="/" class="inline-block mb-6">
          <span class="text-4xl font-display font-bold text-white tracking-wider">AVANTIS</span>
        </RouterLink>
        <h1 class="text-3xl md:text-4xl font-display font-bold text-white uppercase tracking-wide mb-2">Вход в аккаунт</h1>
        <p class="text-gray-500">Введите данные для входа в личный кабинет</p>
      </div>
      ...
      <div class="bg-dark-lighter border border-white/5 p-8 md:p-10 relative overflow-hidden">
        ...
        <form class="space-y-6" @submit.prevent="login">
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input id="password" v-model="form.password" :type="showPassword ? 'text' : 'password'" required class="w-full bg-dark border border-white/10 text-white pl-12 pr-12 py-3.5 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all placeholder-gray-600 text-sm" placeholder="Введите пароль" />
          ...
          <button type="submit" class="btn btn-primary w-full text-lg shadow-lg shadow-primary/20 py-4" :disabled="submitting">
        ...
        <div class="text-center">
          <p class="text-gray-500 text-sm mb-3">Ещё нет аккаунта?</p>
          <RouterLink to="/register" class="btn btn-outline w-full"><span>Зарегистрироваться</span></RouterLink>
        </div>
      </div>
